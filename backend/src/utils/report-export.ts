...
export const generateDailyReportPDF = (report: LedgerReport): Promise<Buffer> => {
  return new Promise((resolve) => {
    ...
    report.rows.slice(0, 50).forEach((row) => {
      doc.fontSize(10).text(
        `${format(row.txnDate, "dd.MM.yyyy")} | ${row.txnNo} | ${row.type}/${row.method} | ${row.description}`,
      );
      doc
        .fontSize(9)
        .text(
          `Karşı Taraf: ${row.contact ?? "-"} | Banka: ${row.bankAccount ?? "-"} | Kart: ${row.card ?? "-"}`,
          { indent: 10 },
        );
      doc
        .fontSize(9)
        .text(
          `Giriş: ${formatCurrency(row.inflow)} | Çıkış: ${formatCurrency(row.outflow)} | Bakiye: ${formatCurrency(row.balance)}`,
          { indent: 10 },
        );
      doc.moveDown(0.5);
    });
...
export const generateDailyReportCSV = (report: LedgerReport): string => {
  const headers = [
    "Tarih",
    "Belge No",
    "Tür",
